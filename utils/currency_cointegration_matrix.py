...
def perform_cointegration_analysis(s, tickers, start_date, period_name):
    df = fetch_data(s, "currency_1d", tickers, start_date)
...
def get_currency_cointegration_data():
    current_dir = os.path.dirname(os.path.abspath(__file__))
    json_file_path = os.path.join(current_dir, "export", "combined_currency_cointegration_results.json")
...
def update_currency_cointegration_data():
    s = connect_to_dolphindb()
    tickers = TICKERS_DICT.get('CURRENCY', [])
    now = datetime.now()
    ...
    output_file_path = os.path.join(export_directory, "combined_currency_cointegration_results.json")

    with open(output_file_path, "w") as json_file:
        json.dump(combined_results, json_file, indent=4)

    print(f"Combined JSON file saved to: {output_file_path}")
    print(f"Currency cointegration data updated at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

if __name__ == '__main__':
    update_currency_cointegration_data()